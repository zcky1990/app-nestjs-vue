<template>
    <div @focusout="handleFocusOut" class="flex flex-col gap-2" tabindex="0">
        <label v-show="showDropdownLabel" :class="['block text-xs font-medium text-gray-700']"> {{ dropdownLabel }}
        </label>
        <div class="dropdown relative">
            <div class="relative block">
                <div
                    :class="['inline-flex items-center overflow-hidden rounded-md border bg-white w-full', getBorderClassInput]">
            ...
            <div :class="['absolute left-0 end-0 z-10 mt-2 divide-y divide-gray-100 rounded-md border border-gray-100 bg-white shadow-lg w-full', showDropdown]"
                role="menu">
                <div class="p-2">
                    <div v-for="item in dropdownItems" :key="item.value"
        ...
        <label v-show="showError" :class="['block text-xs font-light pb-2', getLabelTextColor]"> Please select item from
            dropdown </label>
...
export default defineComponent({
    ...
    data() {
        return {
            dropdownOpen: false,
            selectedItem: 'Please choose' as string,
        }
    },
    computed: {
        showDropdown(): string {
        ...
        selectedValue(): string {
            ...
            this.dropdownItems.forEach(function (item: any) {
    ...
    methods: {
        ...
        handleSelected(item: { key: string, value: string }) {
            this.dropdownOpen = false;
