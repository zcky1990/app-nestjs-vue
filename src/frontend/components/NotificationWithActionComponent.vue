<template>
    <div v-if="showDialog" class="modal fixed inset-0 z-10 overflow-auto bg-black bg-opacity-40" role="dialog">
        <div class="modal-content m-24 mx-auto w-4/5 grid h-3/4 place-content-center">
            <div class="max-w-md p-6 mx-auto h-fit">
                <div class="rounded-2xl border border-blue-100 bg-white p-4 shadow-lg sm:p-6 lg:p-8">
                    ...
                    <div class="mt-6 sm:flex sm:gap-4 justify-center">
                        <button @click="handleSubmit"
                            :class="['inline-block w-2/4 rounded-lg px-5 py-3 text-center text-sm font-light', bgSubmitButton, textColorSubmitButton]">
                            {{ buttonDialog.submit }}
                        </button>
...
export default defineComponent({
    ...
    computed: {
        isNotification(): boolean {
            return this.typeDialog === 'notification';
        },
        bgSubmitButton(): string {
            return this.isNotification ? 'bg-blue-500' : 'bg-red-400';
        },
        textColorSubmitButton(): string {
            return 'text-white';
        }
    },
    methods: {
